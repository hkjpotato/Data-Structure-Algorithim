...
public class GraphAlgorithms {
    ...
    public static Map<Vertex, Integer> dijkstraShortestPath(Graph graph,
        ...
        //VertexDistancePair records the vertex and distance from start
            ...
            if (!visited.contains(currentVertex)) {
                ...
                if (graph.getAdjacencies(currentVertex) != null) {
                    for (Map.Entry<Vertex, Integer> entry
                            : graph.getAdjacencies(currentVertex).entrySet()) {
                        VertexDistancePair newPair =
                                new VertexDistancePair(
                                        entry.getKey(),
                                        entry.getValue() + currentDistance);
                        queue.add(newPair);
                    }
                }
        ...
        for (int k = 0; k < vertexNum; k++) {
            ...
            for (int i = 0; i < vertexNum; i++) {
                for (int j = 0; j < vertexNum; j++) {
                    ...
                    //if a negative diagonal value is found
    ...
    /**
     * A topological sort helper method
     * recursively checks every Vertex using DFS
     * stored the visited Vertex from the smaller one to larger one
     *
     *
     *
     ...
     * @param sorted a listed of sorted vertex
     */
    private static void topoSortHelper(
            Vertex vertex, Graph graph,
            Set<Vertex> visited, List<Vertex> sorted) {
        if (!visited.contains(vertex)) {
            if (graph.getAdjacencies(vertex) != null) {
                for (Map.Entry<Vertex, Integer> entry
                        : graph.getAdjacencies(vertex).entrySet()) {
                    //may need to check if vertex's id != adjacent's id
                    //but for acyclic graph it should be OK
